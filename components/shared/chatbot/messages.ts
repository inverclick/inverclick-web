...
export function generateGoToProjectsMessages() {
  return [
    `¡Hola, {UserName}! ¿Te imaginas disfrutando un cafecito en tu nuevo hogar acá en Colombia? ¡Vamos a hacerlo realidad! Puedes preguntarme por propiedades por ubicación, rango de precios o tipo de inmueble. O si prefieres, agendamos una videollamada con uno de nuestros asesores, ¿qué dices?`,
    `¡Hola, {UserName}! ¿Te imaginas despertar en tu nuevo hogar con esa vista de ensueño? Vamos a hacerlo realidad. Puedes preguntarme por propiedades según ubicación, precio o tipo de inmueble. O si lo prefieres, agendamos una videollamada con uno de nuestros asesores. ¡Tú decides!`,
    `¡Vamos por esa inversión ideal, {UserName}! En Colombia hay oportunidades bacanas que te están esperando. Cuéntame qué te interesa: ubicación, precio o tipo de inmueble, y te enseño las mejores opciones. O si prefieres, agendamos una videollamada con un asesor para revisar todo a detalle, ¿va?`,
    `¡Imagínate, {UserName}, compartiendo momentos inolvidables en tu nuevo hogar! ¿Qué te parece si exploramos algunas opciones? Dime el rango de precios, la ubicación o el tipo de propiedad que buscas y vamos encontrando la ideal. También podemos agendar una videollamada con un asesor experto.`,
    `¡Ahora es el momento perfecto para invertir en Colombia, {UserName}! ¿Te animas a descubrir propiedades en una ubicación específica, en un rango de precios o según el tipo de inmueble? Dime qué buscas y te muestro las mejores opciones. También podemos agendar una videollamada con uno de nuestros asesores para revisar todo con detalle. ¡Estoy aquí para ayudarte!`,
    `¡Aquí arrancamos la búsqueda de tu gran inversión, {UserName}! Cuéntame, ¿qué es lo más importante para ti: ubicación, precio o tipo de propiedad? Con gusto te muestro opciones que se ajusten a lo que buscas, o si prefieres, agendamos una videollamada con un asesor para explorarlo juntos`,
  ];
}
...
export function generateGoToProjectsWithFiltersMessages() {
  return [
    ...
    `¡Aquí te dejo tus resultados, {UserName}! Si quieres ajustar la búsqueda, por ejemplo, expandir el rango de precios, solo dime. También podemos agendar una videollamada para revisar las opciones a fondo.`,
...
export function generateGoToProjectMessages() {
  return [
    `¡Aquí lo tienes, {UserName}! Este es el proyecto [nombre del proyecto], una gran oportunidad de inversión. Cuéntame, ¿quieres conocer más detalles, simular un crédito o prefieres que agendemos una videollamada para hablarlo con un asesor?`,
    ...
    `Aquí tienes el proyecto [nombre del proyecto], {UserName}. Si quieres saber más sobre sus características, cómo financiarlo, ver alternativas similares o agendar una videollamada con uno de nuestros asesores, dime y te echo una mano.`,
    `Te presento el proyecto [nombre del proyecto], {UserName}. ¿Quieres saber más sobre su ubicación, diseño o precios? Si prefieres, también podemos hacer una simulación de crédito o agendar una videollamada para resolver todas tus preguntas.`,
  ];
}
...
export function generateQuestionAboutProjectMessages() {
  return [
    `¡Claro que sí, {UserName}! Este proyecto nos encanta porque [información del proyecto]. ¿Qué más te gustaría saber? ¿O prefieres agendar una videollamada con nuestros asesores?`,
    `{UserName}, este proyecto nos encanta porque combina una ubicación estratégica, diseño innovador y características que se adaptan a lo que buscas. Es ideal para una gran inversión. ¿Qué opinas, te animas a agendar una videollamada con nuestros asesores?`,
    `{UserName}, este es uno de nuestros proyectos favoritos. Su ubicación privilegiada y sus características lo hacen ideal. Estoy aquí para contarte todos los detalles que necesites. ¿Qué más quieres saber de este u otros proyectos? ¿O prefieres agendar una videollamada con nuestros asesores?`,
    `{UserName}, este proyecto combina un diseño excelente con una ubicación privilegiada. Si deseas más detalles o prefieres hablar directamente con un asesor en una videollamada, solo dímelo.`,
    `Por supuesto, {UserName}. Este proyecto ha llamado la atención por su propuesta integral y su potencial de inversión. ¿Te gustaría que te cuente más o prefieres agendar una videollamada con uno de nuestros asesores para conversar en detalle?`,
  ];
}
