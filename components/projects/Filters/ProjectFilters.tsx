  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      <AlertDialogTrigger asChild>
        <Button
          variant="outline"
          className="!text-sm font-normal py-1 h-8 hover:bg-primary-100 hover:text-primary-600 ease-in flex gap-3 relative"
        >
          {hasSearchParams ? (
            <span className="absolute -right-1 -top-1 h-3 w-3 rounded-full bg-primary-700" />
          ) : null}
      ...
      <AlertDialogContent className="p-0 md:min-w-[800px] rounded-lg">
        <header className="flex w-full items-center border-b border-zinc-300 py-3 px-6">
          ...
          />
          <h3 className="flex-1 md:text-lg text-primary-600 text-center font-semibold">
            Filtros
          </h3>
        </header>
        ...
        <AlertDialogFooter className="py-4 px-6 gap-4 !justify-between border-t border-zinc-300">
          <button
            onClick={onClearSearchParams}
            className="text-sm md:text-base font-medium hover:text-primary-600 transition-colors ease-in"
          >
            Quitar filtros
          </button>
          <button
            onClick={onApplyFilters}
            className="bg-primary-600 hover:bg-primary-800 transition-colors ease-in text-white px-4 py-2 rounded-md text-sm md:text-base font-medium"
          >
