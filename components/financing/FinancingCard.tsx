      ...
      <Atropos onClick={handleCardClick} className={`${isFlipped ? 'z-30' : ''} flip-card ${isFlipped ? 'flipped' : ''} flex-1 my-atropos p-2 drop-shadow-2xl cursor-pointer`}>
        <div className="flip-card-inner">
          <div className="flip-card-front bg-white ">
            <p className={`ml-8 lg:mx-10 xl:m-20 text-5xl xl:text-6xl text-left font-semibold text-${color}`}>
              {title[0]}
              <br />
              {title[1]}
            </p>
            <div className={`absolute bottom-8 right-8 bg-${color} rounded-full p-2`}>
              <Plus className="h-8 w-8 text-white" />
            </div>
          </div>
          <div ref={cardRef} className="flip-card-back">
            <p className={`text-6xl font-semibold text-${color}`}>
              {text}
            </p>
          </div>
        </div>      
